...
function twoArrayAndPhrase(n, m, matrix){
    ...
    for(var i = 0; i < n; i++) {
        for(var j = 0; j < m; j++) {
            if(matrix[i][j]=='s') {
                if((i<(n-3))&&(matrix[i+1][j]=='a' && matrix[i+2][j]=='b' && matrix[i+3][j]=='a'))
                    count++;
                if((j<(m-3))&&(matrix[i][j+1]=='a' && matrix[i][j+2]=='b' && matrix[i][j+3]=='a'))
                    count++;
                if((i>=3 && j<(m-3)) && (matrix[i-1][j+1]=='a' && matrix[i-2][j+2]=='b' && matrix[i-3][j+3]=='a'))
                    count++;
                if((i<(n-3) && j<(m-3)) && (matrix[i+1][j+1]=='a' && matrix[i+2][j+2]=='b' && matrix[i+3][j+3]=='a'))
                    count++;
            }
        }
    }
